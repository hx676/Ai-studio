...
from app.core.upload_limits import (
    ARCHIVE_MAX_BYTES,
    ARCHIVE_RESOURCE_MAX_BYTES,
    WORKFLOW_JSON_MAX_BYTES,
    copy_zip_entry_limited,
    ensure_request_upload_size,
    ensure_upload_size,
    ensure_upload_size_sync,
    read_upload_limited,
    read_upload_stream_limited,
    save_upload_to_path_limited,
    validate_graph_size,
...
from app.core.security import configured_origins, is_loopback_host, redact_sensitive_text, websocket_origin_allowed
...
async def websocket_endpoint(websocket: WebSocket, client_id: str = None):
    if not websocket_origin_allowed(websocket):
        await websocket.close(code=1008, reason="Origin or Host is not allowed")
        return
...
# This module is imported as a compatibility runtime by SynCanvas.  Keep all
# upstream storage paths rooted at the project directory, not at ``app/``.
UPSTREAM_SOURCE_COMMIT = "96c00854a7785edda52a52ebd58e1371d6177f2b"
UPSTREAM_SOURCE_VERSION = "2026.07.28.1"
...
def save_storage_settings(payload):
    ...
    atomic_write_json(STORAGE_SETTINGS_FILE, dirs)
...
def load_projects():
    try:
        data = read_json_resilient(PROJECTS_PATH, {})
...
def jimeng_login_fields_from_text(text):
    ...
    if verification_url and not verification_url.startswith(("http://", "https://", "dreamina://")):
        verification_url = ""
...
def jimeng_login_response_fields(text):
    ...
    qr_image_url = ""
    if fields["verification_url"] and not fields["expired"]:
        qr_image_url = f"/api/jimeng/login/qr?v={int(started_at)}"
    return {
        **fields,
        "qr_image_url": qr_image_url,
        # Compatibility for clients that used qr_url as an image source.
        "qr_url": qr_image_url,
    }
    ...
    for attempt in range(attempts):
        try:
            ...
            if attempt + 1 >= attempts:
                raise
            print(redact_sensitive_text(f"[HTTPX-RETRY] {method} {url} transient error: {exc}; retry {attempt + 2}/{attempts}"), flush=True)
            await asyncio.sleep(retry_delay * (attempt + 1))
    if last_exc:
        raise last_exc
...
def prune_generation_history_for_media(paths: List[str]) -> int:
    ...
    try:
        with HISTORY_LOCK:
            ...
            if removed:
                atomic_write_json(HISTORY_FILE, kept, indent=4)
            return removed
    except (OSError, UnicodeError, json.JSONDecodeError):
        return 0
...
def default_asset_library():
    categories = [
        ...
        {"id": "templates", "name": "模板", "type": "template", "default": True, "items": []},
    ]
    return {
...
def normalize_asset_library(lib):
    ...
    for library in libraries:
        ...
        template_category = next((c for c in cats if c.get("type") == "template"), None)
        if not template_category:
            category_id = "templates" if library.get("id") == "default" else f"templates_{library.get('id')}"
            template_category = {"id": category_id[:40], "name": "模板", "type": "template", "default": True, "items": []}
            cats.append(template_category)
        elif not any(c.get("type") == "template" and c.get("default") for c in cats):
            template_category["default"] = True
        for cat in cats:
...
def remove_asset_library_file(item) -> None:
    ...
    try:
        if isinstance(item, dict) and item.get("kind") == "template":
            ...
            if re.fullmatch(r"tmpl_[a-f0-9]{12}", template_id):
                root = os.path.abspath(os.path.join(ASSET_LIBRARY_DIR, "templates"))
                target = os.path.abspath(os.path.join(root, template_id))
...
def _write_local_upload_classification(filename, classification):
    path = _local_upload_classification_path(filename)
    atomic_write_json(path, normalize_asset_classification(classification))
...
def make_workflow_library_item_from_stream(stream, size: int, filename: str, name: str = "") -> Dict[str, Any]:
    ...
    dest_path = os.path.join(ASSET_LIBRARY_DIR, dest_name)
    os.makedirs(ASSET_LIBRARY_DIR, exist_ok=True)
    temp_path = f"{dest_path}.{uuid.uuid4().hex}.tmp"
    stream.seek(0)
    try:
...
def save_asset_url_library(data, broadcast=True):
    with CANVAS_LOCK:
        ...
        os.makedirs(DATA_DIR, exist_ok=True)
        atomic_write_json(ASSET_URL_LIBRARY_PATH, lib)
...
def shared_folders_save(data):
    os.makedirs(DATA_DIR, exist_ok=True)
    atomic_write_json(SHARED_FOLDERS_FILE, data)
...
def save_prompt_libraries(data):
    ...
    os.makedirs(DATA_DIR, exist_ok=True)
    atomic_write_json(PROMPT_LIBRARY_PATH, data)
    return data
...
async def save_ai_image_to_output(image_data, prefix="online_", category="output"):
    ...
    try:
        ...
        print(redact_sensitive_text(f"保存上游图片失败: {e}; url={value}"))
        return value
...
async def generate_gemini_provider_image(prompt, size, model, reference_images=None, provider=None):
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=1800.0, write=120.0, pool=20.0)) as client:
        ...
        if response.status_code in {404, 405} and not explicit_endpoint:
            # New API gateways keep Gemini's native request/response schema,
            # but mount generateContent at /v1/models/... rather than /v1beta.
...
async def upload_ai_reference(files: List[UploadFile] = File(...)):
    ...
    await ensure_request_upload_size(files, per_file=max_upload_bytes)
    for file in files:
        ...
        size = await save_upload_to_path_limited(file, path, max_upload_bytes, file.filename or "file")
        if size <= 0:
            continue
...
async def upload_ai_base64(payload: Base64UploadRequest):
    ...
    if len(raw) > 70 * 1024 * 1024:
        raise HTTPException(status_code=413, detail="编码后的上传内容过大")
...
async def upload_comfyui_base64(payload: Base64UploadRequest):
    ...
    for addr in COMFYUI_INSTANCES:
        try:
            ...
            print(f"ComfyUI base64 upload error for {addr}: {redact_sensitive_text(exc)}")
    if not comfy_name:
            ...
            with open(path, "wb") as output:
                while True:
                    ...
                    if not chunk:
                        break
                    total += len(chunk)
                    if total > ARCHIVE_RESOURCE_MAX_BYTES:
                        raise HTTPException(status_code=413, detail=f"{file.filename or '文件'} 超过 500 MiB")
                    output.write(chunk)
        except Exception:
            try:
                os.unlink(path)
...
async def jimeng_install_start():
    if os.name != "nt":
        raise HTTPException(status_code=400, detail="当前一键安装器仅支持 Windows。")
    candidates = [
        os.path.join(BASE_DIR, "CLI", "windows", "jimeng", "jimeng_cli_install.ps1"),
        os.path.join(BASE_DIR, "tools", "jimeng_cli_install.ps1"),
    ]
    script_path = next((path for path in candidates if os.path.isfile(path)), "")
    if not script_path:
        raise HTTPException(status_code=404, detail="未找到项目内置的即梦 CLI 安装脚本。")
    try:
        ...
        raise HTTPException(status_code=500, detail=f"无法启动即梦 CLI 安装器：{exc}") from exc
    return {
        "success": True,
        "pid": process.pid,
        "message": "安装窗口已打开。请按窗口提示完成 WSL、Dreamina CLI 安装和登录。",
    }
...
async def jimeng_login_start():
    ...
    # Current Dreamina CLI exits immediately in --headless mode and requires a
    # separate `login checklogin` call. Keep the normal login process alive so
    # it polls the device flow and persists the credential after authorization.
    args = ["login"]
    ...
    return {
        ...
        "text": text,
        **jimeng_login_response_fields(text),
        "started_at": JIMENG_LOGIN_SESSION.get("started_at") or 0,
    }

@app.get("/api/jimeng/login/qr")
async def jimeng_login_qr_image():
    fields = jimeng_login_fields_from_text(jimeng_login_text())
    if not fields["verification_url"]:
        raise HTTPException(status_code=404, detail="当前没有可用的即梦登录二维码，请重新点击扫码登录")
    if fields["expired"]:
        raise HTTPException(status_code=410, detail="即梦登录二维码已过期，请重新点击扫码登录")
    image = qrcode.make(fields["verification_url"])
    buffer = BytesIO()
...
async def create_canvas_image_task(payload: OnlineImageRequest):
    ...
    with CANVAS_TASK_LOCK:
        prune_terminal_mapping(CANVAS_TASKS, {"succeeded", "failed", "cancelled", "canceled"})
        active = sum(1 for task in CANVAS_TASKS.values() if task.get("status") in {"queued", "running", "pending", "jimeng_pending"})
...
def download_canvas_assets(payload: CanvasAssetDownloadRequest):
    archive_handle, archive_path = tempfile.mkstemp(prefix="syncanvas-assets-", suffix=".zip")
...
def build_canvas_workflow_archive_file(payload: CanvasWorkflowExportRequest) -> Tuple[str, Dict[str, Any]]:
    ...
    validate_graph_size(nodes_payload, connections_payload)
    handle, archive_path = tempfile.mkstemp(prefix="syncanvas-workflow-", suffix=".zip")
    os.close(handle)
    resources = []
    used = set()
    try:
        with zipfile.ZipFile(archive_path, "w", zipfile.ZIP_DEFLATED) as zf:
            if payload.include_resources:
                for url in canvas_workflow_collect_resource_refs(nodes_payload):
                    ...
                    if resource_size > ARCHIVE_RESOURCE_MAX_BYTES:
                        raise HTTPException(status_code=413, detail=f"资源超过 500 MiB：{os.path.basename(path)}")
                    archive_name = canvas_workflow_unique_archive_name(os.path.basename(path), used)
                    archive_entry = f"resources/{archive_name}"
                    zf.write(path, archive_entry)
                    resources.append({
                        "url": url,
            ...
            raw_workflow = json.dumps(workflow, ensure_ascii=False, indent=2).encode("utf-8")
            if len(raw_workflow) > WORKFLOW_JSON_MAX_BYTES:
                raise HTTPException(status_code=413, detail="工作流 JSON 超过 32 MiB")
            zf.writestr("workflow.json", raw_workflow)
        if os.path.getsize(archive_path) > ARCHIVE_MAX_BYTES:
            raise HTTPException(status_code=413, detail="工作流压缩包超过 512 MiB")
        return archive_path, {
            "resources": resources,
...
async def export_canvas_workflow(payload: CanvasWorkflowExportRequest):
    archive_path, _ = await asyncio.to_thread(build_canvas_workflow_archive_file, payload)
...
async def export_canvas_workflow_to_library(payload: CanvasWorkflowExportRequest):
    archive_path, meta = await asyncio.to_thread(build_canvas_workflow_archive_file, payload)
    filename = sanitize_export_filename(payload.filename or "canvas-workflow.zip", "canvas-workflow.zip")
    if not filename.lower().endswith(".zip"):
        filename += ".zip"
    lib = load_asset_library()
    ...
    try:
        with open(archive_path, "rb") as archive:
            item = await asyncio.to_thread(
                make_workflow_library_item_from_stream,
                archive,
                os.path.getsize(archive_path),
                filename,
                payload.name or os.path.splitext(filename)[0],
            )
    finally:
        if os.path.exists(archive_path):
            os.unlink(archive_path)
...
async def upload_asset_library_workflows(
    ...
    await ensure_request_upload_size(selected_files)
    lib = load_asset_library()
    _, cat = asset_library_workflow_category(lib, library_id, category_id)
    added = []
    for file in selected_files:
        size = await ensure_upload_size(file, ARCHIVE_MAX_BYTES, file.filename or "工作流")
        await file.seek(0)
        signature = await file.read(2)
        await file.seek(0)
        filename = file.filename or "canvas-workflow.zip"
        lower = filename.lower()
        if not (lower.endswith(".json") or lower.endswith(".zip") or signature == b"PK"):
            continue
        if lower.endswith(".json") and size > WORKFLOW_JSON_MAX_BYTES:
            raise HTTPException(status_code=413, detail="工作流 JSON 超过 32 MiB")
        item = await asyncio.to_thread(
            make_workflow_library_item_from_stream,
            file.file,
...
def _import_canvas_workflow_sync(file: UploadFile):
    size = ensure_upload_size_sync(file, ARCHIVE_MAX_BYTES, file.filename or "工作流")
    if not size:
        raise HTTPException(status_code=400, detail="文件为空")
    name = str(file.filename or "").lower()
    file.file.seek(0)
    signature = file.file.read(2)
    file.file.seek(0)
    resource_mapping = {}
    workflow = None
    import_dir = ""
    try:
        if name.endswith(".zip") or signature == b"PK":
            with zipfile.ZipFile(file.file, "r") as zf:
                entries = validate_zip_archive(zf)
                candidates = [n for n in entries if n.lower().endswith("workflow.json")]
                workflow_name = "workflow.json" if "workflow.json" in entries else (candidates[0] if candidates else "")
                if not workflow_name:
                    raise HTTPException(status_code=400, detail="压缩包中没有 workflow.json")
                workflow_info = entries[workflow_name]
                if workflow_info.file_size > WORKFLOW_JSON_MAX_BYTES:
                    raise HTTPException(status_code=413, detail="工作流 JSON 超过 32 MiB")
                workflow_buffer = BytesIO()
                copy_zip_entry_limited(zf, workflow_info, workflow_buffer, WORKFLOW_JSON_MAX_BYTES)
                workflow = json.loads(workflow_buffer.getvalue().decode("utf-8-sig"))
                if not isinstance(workflow, dict):
                    raise HTTPException(status_code=400, detail="ZIP 工作流格式不正确")
                declared = {}
                for res in workflow.get("resources") or []:
                    archive = str((res or {}).get("archive") or "").replace("\\", "/").lstrip("/")
                    if not archive or archive not in entries:
                        raise HTTPException(status_code=400, detail=f"工作流声明的资源不存在：{archive or '(empty)'}")
                    if archive in declared:
                        raise HTTPException(status_code=400, detail=f"工作流重复声明资源：{archive}")
                    declared[archive] = res
                undeclared = [
                    entry_name for entry_name, info in entries.items()
                    if not info.is_dir() and entry_name != workflow_name and entry_name not in declared
                ]
                if undeclared:
                    raise HTTPException(status_code=400, detail=f"压缩包包含未声明资源：{undeclared[0]}")
                stamp = time.strftime("%Y%m%d-%H%M%S")
                import_dir = os.path.join(OUTPUT_INPUT_DIR, f"workflow_import_{stamp}_{uuid.uuid4().hex[:6]}")
                os.makedirs(import_dir, exist_ok=True)
                for archive, res in declared.items():
                    ...
                    with open(target, "wb") as dst:
                        copy_zip_entry_limited(zf, entries[archive], dst, ARCHIVE_RESOURCE_MAX_BYTES)
            ...
            raw = read_upload_stream_limited(file, WORKFLOW_JSON_MAX_BYTES, "工作流 JSON")
...
async def import_canvas_workflow(file: UploadFile = File(...)):
    return await asyncio.to_thread(_import_canvas_workflow_sync, file)
...
def _export_smart_canvas_group_sync(payload: SmartCanvasGroupExportRequest):
...
async def export_smart_canvas_group(payload: SmartCanvasGroupExportRequest):
    return await asyncio.to_thread(_export_smart_canvas_group_sync, payload)
...
async def create_asset_library(payload: AssetLibraryRequest):
    ...
    library["categories"].append({"id": f"templates_{uuid.uuid4().hex[:8]}", "name": "模板", "type": "template", "default": True, "items": []})
...
async def delete_asset_library(library_id: str):
    ...
    for category in (removed_library or {}).get("categories", []):
...
async def create_asset_library_category(payload: AssetLibraryCategoryRequest):
    ...
    if not library:
        raise HTTPException(status_code=404, detail="资产库不存在")
    requested_type = str(payload.type or "").lower()
    cat_type = requested_type if requested_type in {"image", "workflow", "template"} else "image"
...
async def delete_asset_library_category(category_id: str, library_id: str = ""):
    ...
    if cat.get("type") == "template" and cat.get("default"):
        raise HTTPException(status_code=400, detail="默认模板分类不能删除")
...
def _batch_crop_asset_library_items_sync(payload: AssetLibraryBatchCropRequest):
    ids = {str(item) for item in (payload.ids or []) if str(item)}
    if not ids:
        raise HTTPException(status_code=400, detail="没有选择资产")
    lib = load_asset_library()
...
async def batch_crop_asset_library_items(payload: AssetLibraryBatchCropRequest):
    return await asyncio.to_thread(_batch_crop_asset_library_items_sync, payload)
...
def save_runninghub_workflow_store(store):
    os.makedirs(DATA_DIR, exist_ok=True)
    atomic_write_json(RUNNINGHUB_WORKFLOW_STORE_FILE, store)
...
if __name__ == "__main__":
    ...
    bind_host = str(os.getenv("SYNCANVAS_MAIN_HOST", "127.0.0.1") or "").strip()
    if not is_loopback_host(bind_host):
        raise RuntimeError("SynCanvas compatibility runtime only permits a loopback bind address")
    uvicorn.run(app, host=bind_host, port=int(os.getenv("SYNCANVAS_MAIN_PORT", "3000")),
