(function(){
    ...
    const panelIds = isSmart
        ? ['smartPromptTemplatePanel', 'smartWorkflowPanel']
        : ['canvasPromptTemplatePanel', 'canvasWorkflowPanel', 'canvasAssetPanel'];
    ...
    function smartPromptTemplateHtml(template, text, mode){
        ...
        const promptHtml = esc(text).replace(/\r?\n/g, '<br>');
        return `<span class="prompt-template-token" contenteditable="false" data-template-id="${esc(templateId)}" data-template-mode="${esc(mode)}" title="已应用提示词模板：${esc(title)}">${esc(title)}</span><br>${promptHtml}`;
    }
