...
function nearestFreeCardPosition(preferred){
    const CARD_W = 248, CARD_H = 150, GAP_X = 28, GAP_Y = 26, COLS = 4;
    const items = canvasesInProject(currentProjectId).filter(item => item.board_x != null && item.board_y != null);
    const available = point => items.every(item => (
        point.x + CARD_W + GAP_X <= item.board_x
        || item.board_x + CARD_W + GAP_X <= point.x
        || point.y + CARD_H + GAP_Y <= item.board_y
        || item.board_y + CARD_H + GAP_Y <= point.y
    ));
    const base = {x:Math.round(preferred.x), y:Math.round(preferred.y)};
    if(available(base)) return base;
    for(let index = 1; index <= Math.max(32, items.length + 8); index++){
        const column = index % COLS;
        const row = Math.floor(index / COLS);
        const candidate = {
            x:base.x + column * (CARD_W + GAP_X),
            y:base.y + row * (CARD_H + GAP_Y),
        };
        if(available(candidate)) return candidate;
    }
    return {x:base.x, y:base.y + (items.length + 1) * (CARD_H + GAP_Y)};
}
function resetView(){
...
function openCanvas(c){
    ...
    canvasOpening = true;
    document.documentElement.classList.add('canvas-opening');
    ...
    window.location.href = (c.kind === 'smart')
        ? `/static/smart-canvas.html?id=${enc}&project=${project}&v=2026.08.01.4`
        : `/static/canvas.html?id=${enc}&project=${project}&v=2026.08.01.4`;
}
...
function openCreateCard(worldPt){
    closeCreateCard();
    worldPt = nearestFreeCardPosition(worldPt);
    closeCardMenu();
    createKind = 'classic';
