...
let activeTemplateLibraryId = '';
let activeTemplateCategoryId = '';
...
let templateManageMode = false;
let promptManageMode = false;
let assetMoveTarget = '';
let templateMoveTarget = '';
...
let templateTreeEdit = null;
let promptTreeEdit = null;
let pendingTreeDelete = '';
let pendingDeleteTemplateId = '';
let templateDetailLoadingId = '';
let templateDetails = new Map();
let templateDetailErrors = new Map();
let assetLibraryRefreshTimer = null;
let assetLibraryRefreshPromise = null;
let assetLibraryRefreshQueued = false;
...
let canvasAssetSourceFilter = 'all';
...
const SEARCH_INPUT_IDS = new Set(['assetSearch','workflowSearch','templateSearch','promptSearch','localSearch','localUploadSearch','canvasAssetSearch']);
...
function activeTemplateLibrary(){
    const libs = assetLibraries();
    return libs.find(lib => lib.id === activeTemplateLibraryId) || libs[0] || null;
}
...
function templateCategories(lib=activeTemplateLibrary()){
    return (lib?.categories || []).filter(cat => (cat.type || '') === 'template');
}
function templateCategoriesAcrossLibraries(){
    const cats = [];
    assetLibraries().forEach(lib => {
        templateCategories(lib).forEach(cat => cats.push({...cat, __libraryId:lib.id, __libraryName:lib.name || '资产库'}));
    });
    return cats;
}
...
function activeTemplateCategory(){
    const cats = templateCategories();
    return cats.find(cat => cat.id === activeTemplateCategoryId) || cats[0] || null;
}
function workflowCount(){
    return workflowCategories().reduce((sum, cat) => sum + ((cat.items || []).length), 0);
}
function templateCountForLibrary(lib){
    return templateCategories(lib).reduce((sum, cat) => sum + ((cat.items || []).length), 0);
}
...
function currentTemplateItems(){
    const query = templateQuery.trim().toLowerCase();
    const source = query
        ? templateCategories().flatMap(cat => cat.items || [])
        : (activeTemplateCategory()?.items || []);
    return source.filter(item => {
        if(!query) return true;
        return [item.name, item.id, item.json_url, item.source?.skill_id, item.source?.canvas_id].join(' ').toLowerCase().includes(query);
    });
}
...
function canvasAssetViewSubtitle(items){
    ...
    const sourceLabel = canvasAssetSourceFilter === 'all' ? '全部来源' : canvasAssetSourceLabel(canvasAssetSourceFilter, true);
    if(canvas) return `${canvasKindLabel(canvas.kind)} / ${items.length} 个资产 / ${sourceLabel} / ${escapeHtml(canvasAssetSortLabel())}`;
    const activeCat = activeCanvasAssetCategoryInfo();
    return `${Number(activeCat.canvas_count || 0)} 个画布 / ${items.length} 个资产 / ${sourceLabel} / ${escapeHtml(canvasAssetSortLabel())}`;
}
...
function canvasAssetSourceKind(item){
    const nodeType = String(item?.node_type || '').trim().toLowerCase();
    const url = String(item?.url || '').trim().toLowerCase().replace(/\\/g, '/');
    if(nodeType.includes('output') || url.includes('/assets/output/') || url.startsWith('/output/')) return 'output';
    if(
        nodeType.includes('input')
        || ['image','audio','video'].includes(nodeType)
        || url.includes('/assets/input/')
        || url.startsWith('/input/')
    ) return 'input';
    return 'other';
}
function canvasAssetSourceLabel(value, long=false){
    const kind = typeof value === 'string' ? value : canvasAssetSourceKind(value);
    if(kind === 'input') return long ? '输入素材' : '输入';
    if(kind === 'output') return long ? '生成输出' : '输出';
    return long ? '未归类资产' : '其他';
}
function canvasAssetSourceIcon(kind){
    if(kind === 'input') return 'arrow-down-to-line';
    if(kind === 'output') return 'arrow-up-from-line';
    return 'circle-help';
}
function canvasAssetSourceCounts(items){
    const counts = {all:0, input:0, output:0, other:0};
    (items || []).forEach(item => {
        const kind = canvasAssetSourceKind(item);
        counts.all += 1;
...
function canvasAssetScopedItems(){
    const q = String(canvasAssetQuery || '').trim().toLowerCase();
    return uniqueCanvasAssets(canvasAssetsData.items || []).filter(item => {
        ...
        return [
            ...
            item.node_title,
            item.node_type,
            canvasAssetSourceLabel(item),
            canvasAssetSourceLabel(item, true),
...
function currentCanvasAssetItems(){
    let list = canvasAssetScopedItems().filter(item => {
        return canvasAssetSourceFilter === 'all' || canvasAssetSourceKind(item) === canvasAssetSourceFilter;
    });
    list = list.slice();
...
function templateMoveTargets(includeCurrent=false){
    const currentKey = `${activeTemplateLibraryId}::${activeTemplateCategoryId}`;
    return templateCategoriesAcrossLibraries().map(cat => ({
        key:`${cat.__libraryId}::${cat.id}`,
        libraryId:cat.__libraryId,
        categoryId:cat.id,
        label:`${cat.__libraryName || '资产库'} / ${cat.name || '模板'}`
    })).filter(target => includeCurrent || target.key !== currentKey);
}
function normalizeTemplateMoveTarget(){
    const targets = templateMoveTargets();
    if(!targets.some(item => item.key === templateMoveTarget)) templateMoveTarget = targets[0]?.key || '';
    return targets;
}
...
function findTemplateItem(id){
    for(const lib of assetLibraries()) for(const cat of lib.categories || []) {
        if((cat.type || '') !== 'template') continue;
        for(const item of cat.items || []) if(item.id === id) return item;
    }
    return null;
}
...
function selectedTemplate(){
    const items = currentTemplateItems();
    return items.find(item => item.id === selectedTemplateId) || items[0] || null;
}
...
function managedSelectionTarget(target){
    ...
    if(activeTab === 'templates' && templateManageMode){
        const check = target.closest?.('[data-template-check]');
        const card = target.closest?.('[data-template-card]');
        const id = check?.dataset.templateCheck || card?.dataset.templateCard || '';
...
function applyManagedSelection(kind, id){
    if(kind === 'asset'){
        ...
        const selected = toggleSelectionSet(selectedTemplateIds, id);
        selectedTemplateId = selected ? id : (selectedTemplateId === id ? '' : selectedTemplateId);
    } else if(kind === 'prompt'){
...
function normalizeTemplateState(){
    const libs = assetLibraries();
    if(!activeTemplateLibraryId || !libs.some(lib => lib.id === activeTemplateLibraryId)) activeTemplateLibraryId = assetLibrary.active_library_id || libs[0]?.id || '';
    let cats = templateCategories();
    if(!cats.length){
        const fallback = libs.find(lib => templateCategories(lib).length);
        if(fallback){
            activeTemplateLibraryId = fallback.id;
            cats = templateCategories(fallback);
        }
    }
    if(!activeTemplateCategoryId || !cats.some(cat => cat.id === activeTemplateCategoryId)) activeTemplateCategoryId = cats[0]?.id || '';
    const items = currentTemplateItems();
    if(selectedTemplateId && !items.some(item => item.id === selectedTemplateId)) selectedTemplateId = '';
    if(!selectedTemplateId && items.length) selectedTemplateId = items[0].id;
    selectedTemplateIds = new Set([...selectedTemplateIds].filter(id => findTemplateItem(id)));
    normalizeTemplateMoveTarget();
}
...
function normalizeCanvasAssetState(){
    ...
    if(!['all','input','output','other'].includes(canvasAssetSourceFilter)) canvasAssetSourceFilter = 'all';
...
async function loadAll(){
    ...
    activeTemplateLibraryId = (libs.find(lib => lib.id === 'default') || libs[0])?.id || '';
    ...
    templateDetailErrors.clear();
    templateDetailLoadingId = '';
    selectedPromptIds.clear();
    selectedCanvasAssetIds.clear();
    render();
    setStatus('准备就绪');
}
async function refreshAssetLibraryOnly(){
    if(assetLibraryRefreshPromise){
        assetLibraryRefreshQueued = true;
        return assetLibraryRefreshPromise;
    }
    assetLibraryRefreshPromise = (async () => {
        const data = await apiJson('/api/asset-library');
        assetLibrary = data.library || {libraries:[], categories:[]};
        templateDetails.clear();
        templateDetailErrors.clear();
        templateDetailLoadingId = '';
        if(selectedTemplateId && !findTemplateItem(selectedTemplateId)) selectedTemplateId = '';
        selectedTemplateIds = new Set([...selectedTemplateIds].filter(id => findTemplateItem(id)));
        render();
        return assetLibrary;
    })().finally(() => {
        assetLibraryRefreshPromise = null;
        if(assetLibraryRefreshQueued){
            assetLibraryRefreshQueued = false;
            scheduleAssetLibraryRefresh(0);
        }
    });
    return assetLibraryRefreshPromise;
}
function scheduleAssetLibraryRefresh(delay=40){
    clearTimeout(assetLibraryRefreshTimer);
    assetLibraryRefreshTimer = setTimeout(() => {
        assetLibraryRefreshTimer = null;
        refreshAssetLibraryOnly().catch(err => setStatus(err.message || '模板资产库同步失败'));
...
function render(){
    ...
    else if(activeTab === 'templates') renderTemplateManager();
...
function updateSearchQueryFromInput(id, value){
    ...
    else if(id === 'templateSearch') templateQuery = value || '';
...
function clearSearchSelection(id){
    ...
    else if(id === 'templateSearch') selectedTemplateId = '';
...
function renderCanvasAssetSourceFilters(counts){
    const options = [
        {id:'all', label:'全部', icon:'layers-3'},
        {id:'input', label:'输入', icon:canvasAssetSourceIcon('input')},
        {id:'output', label:'输出', icon:canvasAssetSourceIcon('output')}
    ];
    if(Number(counts.other || 0) > 0) options.push({id:'other', label:'其他', icon:canvasAssetSourceIcon('other')});
    return `<div class="canvas-source-bar">
        <div class="canvas-source-filter-label">资产来源</div>
        <div class="canvas-source-filter" role="group" aria-label="筛选输入和输出资产">
            ${options.map(option => `<button class="source-${escapeAttr(option.id)} ${canvasAssetSourceFilter === option.id ? 'active' : ''}" type="button" data-canvas-asset-source="${escapeAttr(option.id)}" title="查看${escapeAttr(option.label)}资产"><i data-lucide="${escapeAttr(option.icon)}"></i><span>${escapeHtml(option.label)}</span><small>${Number(counts[option.id] || 0)}</small></button>`).join('')}
        </div>
        <div class="canvas-source-summary"><span class="source-input">输入素材 ${Number(counts.input || 0)}</span><span class="source-output">生成输出 ${Number(counts.output || 0)}</span>${Number(counts.other || 0) ? `<span class="source-other">其他 ${Number(counts.other || 0)}</span>` : ''}</div>
    </div>`;
}
function renderCanvasAssetsManager(){
    normalizeCanvasAssetState();
    const items = currentCanvasAssetItems();
    const sourceCounts = canvasAssetSourceCounts(canvasAssetScopedItems());
    ...
    root.innerHTML = `
        ...
        <section class="asset-panel asset-content ${canvasAssetManageMode ? 'manage-on' : ''}">
            ...
            ${renderCanvasAssetSourceFilters(sourceCounts)}
            ...
            <div class="content-scroll">
                <div class="asset-grid">
                    ...
                    ${items.length ? '' : `<div class="empty-state">当前画布或来源筛选下没有可下载资产。可以切换“输入 / 输出”筛选，或点击“刷新资源”。</div>`}
                </div>
            </div>
        </section>
...
function renderCanvasAssetGroup(group){
    ...
    const counts = canvasAssetSourceCounts(group.items || []);
    const countSummary = [`${counts.all} 个资产`, counts.input ? `输入 ${counts.input}` : '', counts.output ? `输出 ${counts.output}` : '', counts.other ? `其他 ${counts.other}` : ''].filter(Boolean).join(' · ');
    return `<section class="canvas-asset-group">
        <div class="canvas-asset-group-head">
            ...
            <small>${escapeHtml(countSummary)}</small>
        </div>
...
function renderCanvasAssetCard(item){
    const sourceKind = canvasAssetSourceKind(item);
    return `<article class="asset-card canvas-asset-card source-${escapeAttr(sourceKind)} ${item.id === selectedCanvasAssetId ? 'active' : ''}" data-canvas-asset-card="${escapeAttr(item.id)}">
        ...
        <div class="asset-thumb canvas-asset-thumb">${assetThumb(item)}${renderCanvasAssetKindBadge(item)}${renderCanvasAssetSourceBadge(item)}</div>
        <div class="asset-card-body">
            <div class="asset-card-name" title="${escapeAttr(item.name || '')}">${escapeHtml(item.name || 'canvas asset')}</div>
            <div class="asset-card-meta"><span class="canvas-source-inline source-${escapeAttr(sourceKind)}">${escapeHtml(canvasAssetSourceLabel(sourceKind))}</span><span>${escapeHtml(canvasAssetKindLabel(item))} · ${escapeHtml(item.canvas_title || '未命名画布')}</span></div>
        </div>
    </article>`;
}
function renderCanvasAssetSourceBadge(item){
    const kind = canvasAssetSourceKind(item);
    return `<span class="canvas-source-badge source-${escapeAttr(kind)}" title="${escapeAttr(canvasAssetSourceLabel(kind, true))}"><i data-lucide="${escapeAttr(canvasAssetSourceIcon(kind))}"></i><span>${escapeHtml(canvasAssetSourceLabel(kind))}</span></span>`;
}
...
function renderCanvasAssetDetail(item){
    ...
    return `
        <div class="panel-head">
            <div class="panel-title"><strong>画布资产详情</strong><span>${escapeHtml(canvasAssetKindLabel(item))} · ${escapeHtml(canvasAssetSourceLabel(sourceKind, true))}</span></div>
        ...
        <div class="detail-scroll">
            ...
            <div class="detail-body">
                ...
                <div class="detail-meta-grid">
                    ...
                    <div class="detail-meta canvas-source-detail source-${escapeAttr(sourceKind)}"><span>资产来源</span><strong>${escapeHtml(canvasAssetSourceLabel(sourceKind, true))}</strong></div>
...
function templateValueCount(value){
    if(Array.isArray(value)) return value.length;
    if(value && typeof value === 'object') return Object.keys(value).length;
    return value ? 1 : 0;
}
function templateThumb(item, large=false){
    const url = item?.thumbnail_url || '';
    if(url) return `<img src="${escapeAttr(assetPreviewUrl(url, large ? 720 : 256))}" alt="${escapeAttr(item.name || '模板')}" loading="lazy" decoding="async">`;
    return `<div class="asset-file-icon template-file-icon"><i data-lucide="book-open-check"></i><span>模板 JSON</span></div>`;
}
async function loadTemplateDetail(id, force=false){
    if(!id || (!force && templateDetails.has(id)) || templateDetailLoadingId === id) return templateDetails.get(id) || null;
    if(!force && templateDetailErrors.has(id)) return null;
    templateDetailErrors.delete(id);
    templateDetailLoadingId = id;
    render();
    try {
        const data = await apiJson(`/api/asset-library/templates/${encodeURIComponent(id)}`);
        templateDetails.set(id, data);
        return data;
    } catch(err) {
        templateDetailErrors.set(id, err.message || '读取模板失败');
        throw err;
    } finally {
        if(templateDetailLoadingId === id) templateDetailLoadingId = '';
        render();
    }
}
function renderTemplateManager(){
    ...
    const cached = detail ? templateDetails.get(detail.id) : null;
    const moveTargets = normalizeTemplateMoveTarget();
    root.innerHTML = `
        <aside class="asset-panel asset-nav">
            <div class="panel-head">
                <div class="panel-title"><strong>模板层级</strong><span>按资产库与文件夹管理</span></div>
                <div class="panel-actions compact-actions">
                    ${templateTreeEdit?.placement === 'head' ? '' : '<button class="asset-icon-btn" type="button" data-template-cat-new title="新建模板文件夹"><i data-lucide="folder-plus"></i></button>'}
                    ${renderHeadTreeInlineEdit(templateTreeEdit, 'templateTreeEditInput', 'data-template-tree-edit-save', 'data-template-tree-edit-cancel')}
                </div>
            </div>
            <div class="nav-scroll"><div class="nav-tree">
                ${libs.map(lib => renderTemplateTreeBranch(lib)).join('')}
            </div></div>
        </aside>
        <section class="asset-panel asset-content ${templateManageMode ? 'manage-on' : ''}">
            <div class="content-toolbar">
                <div class="content-heading">
                    <strong>${escapeHtml(cat?.name || '模板资产')}</strong>
                    <span>${escapeHtml(activeTemplateLibrary()?.name || '资产库')} / ${items.length} 个模板</span>
                </div>
                <div class="asset-tools">
                    <label class="asset-search-wrap"><i data-lucide="search"></i><input id="templateSearch" class="asset-search" type="search" value="${escapeAttr(templateQuery)}" placeholder="搜索模板名称或 ID"></label>
                    <button class="asset-btn ${templateManageMode ? 'primary' : ''}" type="button" data-template-manage><i data-lucide="list-checks"></i><span>${templateManageMode ? '完成管理' : '批量管理'}</span></button>
                </div>
            </div>
            <div class="manage-tools">
                <span>已选择 ${selectedTemplateIds.size} 个模板。</span>
                <div class="asset-tools">
                    <button class="asset-btn" type="button" data-template-select-all ${items.length ? '' : 'disabled'}><i data-lucide="check-square"></i><span>全选</span></button>
                    <button class="asset-btn" type="button" data-template-clear-selection ${selectedTemplateIds.size ? '' : 'disabled'}><i data-lucide="square"></i><span>清空</span></button>
                    <select id="templateMoveTarget" class="manage-select" ${moveTargets.length && selectedTemplateIds.size ? '' : 'disabled'}>
                        ${moveTargets.length ? moveTargets.map(target => `<option value="${escapeAttr(target.key)}" ${target.key === templateMoveTarget ? 'selected' : ''}>${escapeHtml(target.label)}</option>`).join('') : '<option value="">没有其他模板文件夹</option>'}
                    </select>
                    <button class="asset-btn" type="button" data-template-move-selected ${moveTargets.length && selectedTemplateIds.size ? '' : 'disabled'}><i data-lucide="folder-input"></i><span>移动</span></button>
                    <button class="asset-btn danger ${pendingBatchDelete === 'template' ? 'detail-confirm' : ''}" type="button" data-template-delete-selected ${selectedTemplateIds.size ? '' : 'disabled'}><i data-lucide="trash-2"></i><span>${pendingBatchDelete === 'template' ? '确认删除' : '删除所选'}</span></button>
                </div>
            </div>
            <div class="content-scroll"><div class="asset-grid template-asset-grid">
                ${items.map(item => renderTemplateCard(item)).join('')}
                ${items.length ? '' : `<div class="empty-state">${templateQuery.trim() ? '没有匹配模板。' : '这里还没有模板。请在智能画布中将蒸馏结果连接到“存模板”节点并保存。'}</div>`}
            </div></div>
        </section>
        <aside class="asset-panel asset-detail">
            ${renderTemplateDetail(detail, cached)}
        </aside>
    `;
    if(detail && !cached && templateDetailLoadingId !== detail.id && !templateDetailErrors.has(detail.id)){
        queueMicrotask(() => loadTemplateDetail(detail.id).catch(err => setStatus(err.message || '读取模板失败')));
    }
}
function renderTemplateTreeBranch(lib){
    const categories = templateCategories(lib);
    const isActiveLib = lib.id === activeTemplateLibraryId;
    const total = templateCountForLibrary(lib);
    return `<div class="tree-branch expanded">
        <button class="tree-row tree-parent ${isActiveLib ? 'contains-active' : ''}" type="button" data-template-lib="${escapeAttr(lib.id)}">
        ...
        <div class="tree-children">
            ${categories.length ? categories.map(cat => {
                const active = isActiveLib && cat.id === activeTemplateCategoryId;
                return `<button class="tree-row tree-child ${active ? 'active' : ''}" type="button" data-template-cat="${escapeAttr(cat.id)}" data-template-cat-lib="${escapeAttr(lib.id)}">
                    <span class="tree-elbow"></span><span class="tree-row-icon"><i data-lucide="book-open-check"></i></span>
                    <span class="tree-row-name">${escapeHtml(cat.name || '模板')}</span><span class="tree-row-count">${(cat.items || []).length}</span>
                </button>${active ? renderTemplateTreeActionBar(cat) : ''}`;
            }).join('') : '<div class="tree-empty">暂无模板文件夹</div>'}
        </div>
    </div>`;
}
function renderTemplateTreeActionBar(cat){
    if(templateTreeEdit && templateTreeEdit.placement !== 'head') return renderTemplateTreeInlineEdit();
    const deleteKey = `template-cat:${activeTemplateLibraryId}:${cat.id}`;
    return `<div class="tree-action-bar child-actions">
        <button type="button" data-template-cat-rename><i data-lucide="pencil"></i><span>重命名</span></button>
        ${cat.default ? '<span><i data-lucide="shield-check"></i>默认文件夹</span>' : `<button type="button" class="danger ${pendingTreeDelete === deleteKey ? 'detail-confirm' : ''}" data-template-cat-delete><i data-lucide="trash-2"></i><span>${pendingTreeDelete === deleteKey ? '确认删除' : '删除'}</span></button>`}
    </div>`;
}
function renderTemplateTreeInlineEdit(){
    if(!templateTreeEdit || templateTreeEdit.placement === 'head') return '';
    return `<div class="tree-inline-edit child-actions">
        <input id="templateTreeEditInput" type="text" value="${escapeAttr(templateTreeEdit.value || '')}" placeholder="模板文件夹名称">
        <button type="button" class="primary" data-template-tree-edit-save><i data-lucide="check"></i><span>保存</span></button>
        <button type="button" data-template-tree-edit-cancel><i data-lucide="x"></i><span>取消</span></button>
    </div>`;
}
function renderTemplateCard(item){
    return `<article class="asset-card template-card ${item.id === selectedTemplateId ? 'active' : ''}" data-template-card="${escapeAttr(item.id)}">
        <input class="asset-card-check" type="checkbox" data-template-check="${escapeAttr(item.id)}" ${selectedTemplateIds.has(item.id) ? 'checked' : ''}>
        <div class="asset-thumb template-thumb">${templateThumb(item)}</div>
        <div class="asset-card-body">
            <div class="asset-card-name" title="${escapeAttr(item.name || '')}">${escapeHtml(item.name || '模板')}</div>
            <div class="asset-card-meta">${(item.reference_image_urls || []).length} 张参考图 · ${escapeHtml(formatDate(item.updated_at || item.created_at))}</div>
        </div>
    </article>`;
}
function renderTemplateDetail(item, cached){
    if(!item) return `<div class="panel-head"><div class="panel-title"><strong>模板预览</strong><span>选择一个模板查看详情</span></div></div><div class="detail-scroll"><div class="detail-empty"><i data-lucide="book-open-check"></i><span>暂无模板资产</span></div></div>`;
    const template = cached?.template || null;
    const loadError = templateDetailErrors.get(item.id) || '';
    const references = item.reference_image_urls || [];
    ...
    const jsonText = template ? JSON.stringify(template, null, 2) : '';
    const targets = templateMoveTargets(true);
    const currentKey = `${activeTemplateLibraryId}::${item.category_id || activeTemplateCategoryId}`;
    return `
        <div class="panel-head">
            <div class="panel-title"><strong>模板预览</strong><span>${templateDetailLoadingId === item.id ? '正在读取 JSON' : loadError || (template ? '已读取最新版本' : '等待读取')}</span></div>
            <div class="panel-actions">
                <button class="asset-icon-btn" type="button" data-template-refresh="${escapeAttr(item.id)}" title="刷新模板"><i data-lucide="refresh-cw"></i></button>
                <button class="asset-icon-btn" type="button" data-template-copy-json="${escapeAttr(item.id)}" ${template ? '' : 'disabled'} title="复制模板 JSON"><i data-lucide="copy"></i></button>
                <button class="asset-icon-btn danger ${pendingDeleteTemplateId === item.id ? 'detail-confirm' : ''}" type="button" data-template-delete="${escapeAttr(item.id)}" title="${pendingDeleteTemplateId === item.id ? '再次点击确认删除' : '删除'}"><i data-lucide="trash-2"></i></button>
            </div>
        </div>
        <div class="detail-scroll">
            <div class="detail-media"><div class="detail-media-frame template-detail-frame">${templateThumb(item, true)}</div></div>
            ${references.length ? `<div class="template-reference-grid">${references.map((url, index) => `<img src="${escapeAttr(assetPreviewUrl(url, 180))}" alt="参考图 ${index + 1}" loading="lazy" decoding="async">`).join('')}</div>` : ''}
            <div class="detail-body">
                <input class="detail-name-input" data-template-inline-name="${escapeAttr(item.id)}" type="text" value="${escapeAttr(item.name || '模板')}" title="直接修改名称">
                <label class="template-move-field"><span>所在文件夹</span><select data-template-move-single="${escapeAttr(item.id)}">${targets.map(target => `<option value="${escapeAttr(target.key)}" ${target.key === currentKey ? 'selected' : ''}>${escapeHtml(target.label)}</option>`).join('')}</select></label>
                <div class="detail-meta-grid">
                    <div class="detail-meta"><span>特征</span><strong>${template ? templateValueCount(features) : '-'}</strong></div>
                    <div class="detail-meta"><span>页面风格</span><strong>${template ? templateValueCount(pages) : '-'}</strong></div>
                    <div class="detail-meta"><span>参考图</span><strong>${references.length}</strong></div>
                    <div class="detail-meta"><span>更新时间</span><strong>${escapeHtml(formatDate(item.updated_at || item.created_at))}</strong></div>
                </div>
                ${prompt ? `<section class="template-prompt-preview"><strong>风格提示词</strong><p>${escapeHtml(prompt)}</p></section>` : ''}
                <section class="template-json-card">
                    <div class="template-json-head"><strong>模板 JSON</strong><span>${jsonText ? `${jsonText.length} 字符` : '读取中'}</span></div>
                    <pre>${escapeHtml(jsonText || loadError || '正在读取模板 JSON...')}</pre>
                </section>
                <div class="detail-url">${escapeHtml(item.id || '')}</div>
            </div>
        </div>
    `;
}
...
async function saveTemplateInlineName(id, name){
    const item = findTemplateItem(id);
    const value = String(name || '').trim();
    if(!item || !value || value === item.name) return;
    const data = await apiJson(`/api/asset-library/templates/${encodeURIComponent(id)}`, {
        method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({name:value})
    });
    assetLibrary = data.library || assetLibrary;
    if(templateDetails.has(id)) templateDetails.set(id, {...templateDetails.get(id), item:data.item});
    render();
    setStatus('模板名称已保存');
}
async function moveTemplateItems(ids, targetKey){
    const target = templateMoveTargets(true).find(item => item.key === targetKey);
    const uniqueIds = [...new Set(ids || [])].filter(id => findTemplateItem(id));
    if(!target || !uniqueIds.length) return;
    let lastData = null;
    for(const id of uniqueIds){
        lastData = await apiJson(`/api/asset-library/templates/${encodeURIComponent(id)}`, {
            method:'PATCH', headers:{'Content-Type':'application/json'},
            body:JSON.stringify({library_id:target.libraryId, category_id:target.categoryId})
        });
    }
    if(lastData?.library) assetLibrary = lastData.library;
    selectedTemplateIds.clear();
    selectedTemplateId = '';
    render();
    setStatus(`已移动 ${uniqueIds.length} 个模板`);
}
async function deleteTemplateItem(id){
    if(!findTemplateItem(id)) return;
    if(pendingDeleteTemplateId !== id){
        pendingDeleteTemplateId = id;
        pendingBatchDelete = '';
        render();
        setStatus('再次点击确认删除模板');
        return;
    }
    const data = await apiJson(`/api/asset-library/templates/${encodeURIComponent(id)}`, {method:'DELETE'});
    assetLibrary = data.library || assetLibrary;
...
async function deleteSelectedTemplates(){
    const ids = [...selectedTemplateIds].filter(id => findTemplateItem(id));
    if(!ids.length) return;
    if(pendingBatchDelete !== 'template'){
        pendingBatchDelete = 'template';
        pendingDeleteTemplateId = '';
        render();
        setStatus(`再次点击确认删除 ${ids.length} 个模板`);
        return;
    }
    let lastData = null;
    for(const id of ids){
        lastData = await apiJson(`/api/asset-library/templates/${encodeURIComponent(id)}`, {method:'DELETE'});
        templateDetails.delete(id);
        templateDetailErrors.delete(id);
    }
    if(lastData?.library) assetLibrary = lastData.library;
...
async function handleClick(event){
    ...
    if(tabBtn){ activeTab = tabBtn.dataset.tab || 'assets'; selectedAssetIds.clear(); selectedWorkflowIds.clear(); selectedTemplateIds.clear(); selectedPromptIds.clear(); selectedLocalIds.clear(); selectedLocalUploadIds.clear(); selectedCanvasAssetIds.clear(); render(); return; }
    ...
    const canvasAssetSource = target.closest?.('[data-canvas-asset-source]');
    if(canvasAssetSource){
        canvasAssetSourceFilter = canvasAssetSource.dataset.canvasAssetSource || 'all';
    ...
    if(target.closest?.('[data-template-manage]')){
        templateManageMode = !templateManageMode;
        if(!templateManageMode) selectedTemplateIds.clear();
        pendingDeleteTemplateId = '';
        pendingBatchDelete = '';
        render();
        return;
    }
    if(target.closest?.('[data-template-select-all]')){ currentTemplateItems().forEach(item => selectedTemplateIds.add(item.id)); pendingBatchDelete = ''; render(); return; }
    if(target.closest?.('[data-template-clear-selection]')){ selectedTemplateIds.clear(); pendingBatchDelete = ''; render(); return; }
    if(target.closest?.('[data-template-move-selected]')){ await moveTemplateItems([...selectedTemplateIds], templateMoveTarget); return; }
    if(target.closest?.('[data-template-delete-selected]')){ await deleteSelectedTemplates(); return; }
    const templateRefresh = target.closest?.('[data-template-refresh]');
    if(templateRefresh){ await loadTemplateDetail(templateRefresh.dataset.templateRefresh || '', true); setStatus('模板已刷新'); return; }
    const templateCopy = target.closest?.('[data-template-copy-json]');
    if(templateCopy){
        const id = templateCopy.dataset.templateCopyJson || '';
        const data = templateDetails.get(id) || await loadTemplateDetail(id);
        const ok = await copyTextToClipboard(data?.template ? JSON.stringify(data.template, null, 2) : '');
        setStatus(ok ? '已复制模板 JSON' : '复制失败');
        return;
    }
    const templateDelete = target.closest?.('[data-template-delete]');
    if(templateDelete){ await deleteTemplateItem(templateDelete.dataset.templateDelete || ''); return; }
    if(target.closest?.('[data-template-cat-new]')){
        templateTreeEdit = {kind:'category-new', placement:'head', value:'新模板文件夹', label:'模板文件夹名称'};
        pendingTreeDelete = '';
        render();
        focusTreeEditInput('templateTreeEditInput');
        return;
    }
    if(target.closest?.('[data-template-cat-rename]')){
        const cat = activeTemplateCategory();
        if(!cat) return;
        templateTreeEdit = {kind:'category-rename', value:cat.name || '', label:'模板文件夹名称', categoryId:cat.id};
        pendingTreeDelete = '';
        render();
        focusTreeEditInput('templateTreeEditInput');
        return;
    }
    if(target.closest?.('[data-template-cat-delete]')){ await deleteTemplateCategory(); return; }
    const templateLib = target.closest?.('[data-template-lib]');
    if(templateLib){
        activeTemplateLibraryId = templateLib.dataset.templateLib || '';
        activeTemplateCategoryId = templateCategories()[0]?.id || '';
        selectedTemplateId = '';
        selectedTemplateIds.clear();
        pendingDeleteTemplateId = '';
        render();
    ...
    const templateCat = target.closest?.('[data-template-cat]');
    if(templateCat){
        activeTemplateLibraryId = templateCat.dataset.templateCatLib || activeTemplateLibraryId;
        activeTemplateCategoryId = templateCat.dataset.templateCat || '';
        selectedTemplateId = '';
        selectedTemplateIds.clear();
        pendingDeleteTemplateId = '';
        render();
    ...
    const templateCard = target.closest?.('[data-template-card]');
    if(templateCard){
        selectedTemplateId = templateCard.dataset.templateCard || '';
        pendingDeleteTemplateId = '';
        render();
        return;
    }
    ...
    if(target.closest?.('[data-template-tree-edit-save]')){ await saveTemplateTreeEdit(); return; }
    if(target.closest?.('[data-template-tree-edit-cancel]')){ templateTreeEdit = null; render(); return; }
...
async function saveTemplateTreeEdit(){
    if(!templateTreeEdit) return;
    const name = document.getElementById('templateTreeEditInput')?.value || '';
    if(!String(name || '').trim()){
        setStatus('名称不能为空');
        return;
    }
    if(templateTreeEdit.kind === 'category-new'){
        const lib = activeTemplateLibrary();
        if(!lib){ setStatus('请先选择资产库'); return; }
        activeTemplateLibraryId = lib.id || activeTemplateLibraryId;
        const data = await apiJson('/api/asset-library/categories', {
            method:'POST', headers:{'Content-Type':'application/json'},
            body:JSON.stringify({library_id:activeTemplateLibraryId, name, type:'template'})
        });
        assetLibrary = data.library || assetLibrary;
        activeTemplateCategoryId = data.category?.id || activeTemplateCategoryId;
    } else if(templateTreeEdit.kind === 'category-rename'){
        const categoryId = templateTreeEdit.categoryId || activeTemplateCategory()?.id || activeTemplateCategoryId;
        if(!categoryId) return;
        const data = await apiJson(`/api/asset-library/categories/${encodeURIComponent(categoryId)}`, {
            method:'PATCH', headers:{'Content-Type':'application/json'},
            body:JSON.stringify({name, library_id:activeTemplateLibraryId})
        });
        assetLibrary = data.library || assetLibrary;
        activeTemplateCategoryId = categoryId;
    }
    templateTreeEdit = null;
    pendingTreeDelete = '';
    render();
    setStatus('模板文件夹已保存');
}
...
async function deleteTemplateCategory(){
    const cat = activeTemplateCategory();
    if(!cat) return;
    if(cat.default){ setStatus('默认模板文件夹不能删除'); return; }
    const key = `template-cat:${activeTemplateLibraryId}:${cat.id}`;
    if(pendingTreeDelete !== key){
        pendingTreeDelete = key;
        templateTreeEdit = null;
        render();
        setStatus('再次点击确认删除模板文件夹及其中模板');
        return;
    }
    const ids = (cat.items || []).map(item => item.id);
    const data = await apiJson(`/api/asset-library/categories/${encodeURIComponent(cat.id)}?library_id=${encodeURIComponent(activeTemplateLibraryId)}`, {method:'DELETE'});
    assetLibrary = data.library || assetLibrary;
    ids.forEach(id => { templateDetails.delete(id); templateDetailErrors.delete(id); });
    activeTemplateCategoryId = '';
    selectedTemplateId = '';
    selectedTemplateIds.clear();
    pendingTreeDelete = '';
...
document.addEventListener('keydown', event => {
    ...
    if(event.target?.id === 'templateTreeEditInput'){
        if(event.key === 'Enter'){ event.preventDefault(); saveTemplateTreeEdit().catch(err => setStatus(err.message || '保存失败')); }
        if(event.key === 'Escape'){ event.preventDefault(); templateTreeEdit = null; render(); }
    }
...
root.addEventListener('change', event => {
    ...
    const inlineTemplateName = event.target.closest?.('[data-template-inline-name]');
    if(inlineTemplateName){
        saveTemplateInlineName(inlineTemplateName.dataset.templateInlineName || '', inlineTemplateName.value || '').catch(err => setStatus(err.message || '保存失败'));
        return;
    }
    const templateMoveSingle = event.target.closest?.('[data-template-move-single]');
    if(templateMoveSingle){
        moveTemplateItems([templateMoveSingle.dataset.templateMoveSingle || ''], templateMoveSingle.value || '').catch(err => setStatus(err.message || '移动失败'));
        return;
    }
    ...
    if(event.target?.id === 'templateMoveTarget'){
        templateMoveTarget = event.target.value || '';
        pendingBatchDelete = '';
        render();
    }
...
window.addEventListener('message', event => {
    ...
    if(event.data?.type === 'asset_library_updated') scheduleAssetLibraryRefresh();
    if(event.data?.type === 'studio-route-active' && event.data?.page === 'assets') scheduleAssetLibraryRefresh(0);
});
