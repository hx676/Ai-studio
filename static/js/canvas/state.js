        ...
        const outputLightboxAudio = document.getElementById('outputLightboxAudio');
        ...
        let outputLightboxAudioSyncCleanup = null;
        ...
        function resolveImageModelForProvider(model, providerId){
            const models = providerImageModels(providerId);
            return models.includes(model) ? model : (models[0] || '');
        }
        function normalizeGeneratorApiSelection(node){
            if(!node || node.type !== 'generator') return false;
            const nextProvider = resolveImageProviderId(node.apiProvider || '');
            const nextModel = resolveImageModelForProvider(node.model, nextProvider);
            const changed = node.apiProvider !== nextProvider || node.model !== nextModel;
            node.apiProvider = nextProvider;
            node.model = nextModel;
            return changed;
        }
        async function ensureGeneratorApiSelectionFresh(node){
            await loadConfig();
            const changed = normalizeGeneratorApiSelection(node);
            if(changed) scheduleSave();
            if(typeof render === 'function') render();
            if(!node?.apiProvider || !node?.model){
                throw new Error(tr('canvas.noApiProvidersHint') || tr('canvas.noImageModelsHint') || tr('canvas.apiFailed'));
            }
        }
        ...
        function providerVideoModels(providerId){
            // 不走 providerById（会 fallback 到第一个 provider，造成串台），直接查精确匹配。
            // 平台没单独配置视频模型时，回退到全局视频模型，避免节点下拉误显示为空。
            const provider = apiProviders.find(p => p.id === providerId);
            const providerModels = uniqueModels(provider?.video_models || []);
            if(providerModels.length) return providerModels;
            const globalModels = uniqueModels(videoModels || []);
            return globalModels.length ? globalModels : uniqueModels(DEFAULT_VIDEO_MODELS);
        }
        ...
        async function generatorSizePayloadForRun(gen, refs){
            ...
            const ratio = (gen.ratio === 'source' && !gen.customRatio)
                ? 'square'
                : (gen.ratio ?? 'square');
            return apiImageSizePayload(ratio, gen.resolution || '1k', gen.customRatio || '', gen.customSize || '');
        }
        async function generatorSizeForRun(gen, refs){
            return (await generatorSizePayloadForRun(gen, refs)).size;
        }
        ...
        function imageModelOptions(selectedModel, providerId){
            ...
            const selectedValue = resolveImageModelForProvider(selectedModel, providerId);
            return models.map(model => `<option value="${escapeHtml(model)}" ${model === selectedValue ? 'selected' : ''}>${escapeHtml(model)}</option>`).join('');
        }
        ...
        async function saveCanvas(options={}){
            ...
            if(savingCanvasNow){
                ...
                if(throwOnError){
                    ...
                    while(savingCanvasNow && Date.now() - startedAt < 10000){
                        await new Promise(resolve => setTimeout(resolve, 80));
                    }
                    if(savingCanvasNow) throw new Error('Canvas save is still running');
                    return saveCanvas(options);
                }
                return false;
            }
            ...
            try {
                ...
                if(res.status === 409){
                    ...
                    if(localCanvasDirty || saveCanvasAgain){
                        ...
                        saveCanvasAgain = true;
                        setStatus('Saving...');
                        if(throwOnError) throw new Error('Canvas changed remotely while saving');
                        return false;
                    }
        ...
        async function uploadAudioFiles(files, point){
            if(!ensureCanvas()) return;
            const audios = [...files].filter(isAudioFile);
            if(!audios.length) return;
            const form = new FormData();
            audios.forEach(file => form.append('files', file));
            const data = await fetch('/api/canvas-media/upload', {method:'POST', body:form}).then(async r => {
                if(!r.ok) throw new Error(await responseErrorMessage(r, tr('canvas.audioUploadFailed')));
                return r.json();
            });
            const base = point || screenToWorld(window.innerWidth / 2, window.innerHeight / 2);
            (data.files || []).forEach((file, i) => {
                nodes.push({id:uid('aud'), type:'audio', x:base.x + i * 36, y:base.y + i * 36, url:file.url, name:file.name, mime:file.mime || ''});
            });
            render();
            scheduleSave();
        }
        ...
        function audioUrlFromDataTransfer(dataTransfer){
            if(!dataTransfer) return '';
            const values = [
                dataTransfer.getData?.('text/uri-list') || '',
                dataTransfer.getData?.('text/plain') || ''
            ...
            const html = dataTransfer.getData?.('text/html') || '';
            const audioMatch = html.match(/<(?:audio|source)[^>]+src=["']([^"']+)["']/i);
            const candidates = [
                audioMatch?.[1] || '',
                ...values.split(/\r?\n/)
            ].map(s => String(s || '').trim()).filter(Boolean);
        ...
        function createAudioCardFromUrl(url, point, name='audio'){
        ...
        async function fillAudioNode(nodeId, files){
            if(!ensureCanvas()) return;
            const audios = [...files].filter(isAudioFile);
            if(!audios.length) return;
            const form = new FormData();
            form.append('files', audios[0]);
            const data = await fetch('/api/canvas-media/upload', {method:'POST', body:form}).then(async r => {
                if(!r.ok) throw new Error(await responseErrorMessage(r, tr('canvas.audioUploadFailed')));
                return r.json();
            });
        ...
        function clearAudioNode(nodeId, event=null){
            if(event){
                event.preventDefault();
                event.stopPropagation();
                event.stopImmediatePropagation?.();
        ...
        function renderNode(node){
            ...
            if(node.type === 'audio') {
                if(node.url) {
                    body.innerHTML = `
                        <div class="audio-node-body">
                            <div class="audio-node-icon"><i data-lucide="audio-lines" class="w-6 h-6"></i></div>
                            <div class="audio-node-name" title="${escapeAttr(node.name || mediaDisplayName(node.url, 'audio'))}">${escapeHtml(node.name || mediaDisplayName(node.url, 'audio'))}</div>
                            <audio class="audio-player" src="${escapeAttr(node.url)}" controls preload="metadata"></audio>
                            <div class="audio-node-actions">
                                <button type="button" class="preview-text-btn secondary audio-replace"><i data-lucide="file-up" class="w-3.5 h-3.5"></i><span>${tr('common.replace') || 'Replace'}</span></button>
                                <button type="button" class="preview-text-btn secondary audio-clear"><i data-lucide="x" class="w-3.5 h-3.5"></i><span>${tr('common.clear') || 'Clear'}</span></button>
                            </div>
                        </div>
                    `;
                    body.querySelector('.audio-replace').onclick = e => { e.stopPropagation(); pickAudioForNode(node.id); };
                    body.querySelector('.audio-clear').onclick = e => clearAudioNode(node.id, e);
                } else {
                    body.innerHTML = `<div class="blank-audio"><i data-lucide="audio-lines" class="w-7 h-7"></i><div class="text-[11px] font-bold">${tr('canvas.clickDragPasteAudio')}</div></div>`;
                    body.querySelector('.blank-audio').onclick = () => pickAudioForNode(node.id);
                }
                const dropTarget = body.querySelector('.audio-node-body') || body.querySelector('.blank-audio');
                if(dropTarget){
                    dropTarget.ondragover = e => {
                        if(hasAudioDropData(e.dataTransfer)){
                            e.preventDefault();
                            e.stopPropagation();
                    ...
                    dropTarget.ondrop = e => {
                        if(!hasAudioDropData(e.dataTransfer)) return;
                        e.preventDefault();
                        e.stopPropagation();
                        dropTarget.classList.remove('drag-over');
                        dropOverlay.classList.remove('active');
                        const files = audioFilesFromDataTransfer(e.dataTransfer);
                        if(files.length) fillAudioNode(node.id, files).catch(err => showErrorModal(err.message || tr('canvas.audioUploadFailed')));
                        else {
                            const droppedUrl = audioUrlFromDataTransfer(e.dataTransfer);
                            if(droppedUrl){
                                node.url = droppedUrl;
                                node.name = mediaDisplayName(droppedUrl, 'audio');
                                node.mime = '';
                                render();
                                scheduleSave();
                            }
        ...
        function refreshOutputNodeContent(node){
            ...
            const items = [
                ...outputDisplayItems(node).map(item => ({
            ...
            if(node.type === 'output'){
                return (node.images || [])
                    ...
                    .map((item, i) => {
                        ...
                        return {url, name:outputImageName(url) || `output-${i + 1}.png`};
                    });
            }
            if(CANVAS_IMAGE_OUTPUT_TYPES.includes(node.type)) return generatedImageRefs(node);
            return [];
        }
        function audioRefsFromNode(node){
            if(!node) return [];
            if(node.type === 'audio' && node.url) return [{url:node.url, name:node.name || mediaDisplayName(node.url, 'audio'), mime:node.mime || ''}];
            if(node.type === 'output'){
                return outputDisplayItems(node)
                    .filter(item => mediaKind(item) === 'audio')
                    .map(item => ({url:outputUrlValue(item), name:item.name || mediaDisplayName(outputUrlValue(item), 'audio'), mime:item.mime || ''}))
                    .filter(ref => ref.url);
            }
            return [];
        }
        ...
        function renderGeneratorBody(node){
            ...
            const imageInputs = ordered.filter(src => src.refs?.length);
            const promptInputs = ordered.filter(src => src.prompt && !src.refs?.length);
            normalizeGeneratorApiSelection(node);
            wrap.innerHTML = `
        ...
        function renderVideoBody(node){
            ...
            const availableVideoModels = providerVideoModels(node.apiProvider);
            if(availableVideoModels.length && !availableVideoModels.includes(node.model)){
                node.model = availableVideoModels[0];
            }
        ...
        function renderVideoAudioInputs(list, node, audioInputs){
            if(!list) return;
            list.innerHTML = audioInputs.length ? '' : `<div class="text-[11px] text-gray-300 py-2">${tr('canvas.inputAudiosEmpty')}</div>`;
            audioInputs.forEach((src, i) => {
                const item = document.createElement('div');
                item.className = 'input-item video-input-item audio-input-item';
                item.draggable = true;
                item.dataset.sourceId = src.id;
                const ref = src.audioRefs?.[0] || {};
                item.innerHTML = `
                    <div class="video-input-thumb audio-input-thumb">
                        <span class="input-index">${i + 1}</span>
                        <i data-lucide="audio-lines" class="w-5 h-5 text-slate-500"></i>
                        <span class="input-label">${escapeHtml(src.label || ref.name || 'audio')}</span>
                    </div>
                `;
                item.ondragstart = e => { e.stopPropagation(); internalDrag = true; e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('application/x-canvas-input', src.id); };
                item.ondragend = () => { internalDrag = false; };
        ...
        async function runGenerator(genId, opts={}){
            ...
            const sizePayload = await generatorSizePayloadForRun(gen, refs);
            const payload = {
                prompt: prompt || 'Edit the reference images.',
                provider_id:gen.apiProvider,
                model:gen.model,
                ...sizePayload,
        ...
        async function runVideoNode(nodeId, opts={}){
            ...
            try {
                ...
                const outputItems = Array.isArray(result.media) && result.media.length ? result.media.filter(item => outputUrlValue(item)) : [
                    ...(result.videos || []).filter(Boolean).map(url => ({url, kind:'video'})),
                    ...(result.audios || []).filter(Boolean).map(url => ({url, kind:'audio'}))
                ];
                const outputUrls = outputItems.map(outputUrlValue).filter(Boolean);
                if(!outputUrls.length){
                    throw new Error(result?.debug
                        ? `视频接口没有返回视频。${JSON.stringify(result.debug)}`
                        : '视频接口没有返回视频，请查看生成日志或后端日志。');
                }
                run.request = requestMetaFromResult(result);
                appendOutputImages(out, outputItems, refs[0], [meta]);
                mergeGeneratedOutputs(node, outputItems, Boolean(opts.cascade));
                addGenerationLog({run, outputs:outputItems, runMs:meta.runMs || 0});
                node.runStatus = 'done'; node.runError = '';
                node.running = false;
                refreshRunNodes(node, out);
                clearTimeout(saveTimer);
                await saveCanvas({throwOnError:true});
            } catch(err) {
                const meta = collectRunMeta(out, pendingId);
                ...
                scheduleSave();
                if(opts.cascade) throw err;
                showErrorModal(err.message || tr('canvas.videoFailed'), tr('canvas.videoFailed'));
            } finally {
                node.running = false;
                refreshRunNodes(node, out);
            }
        ...
        function retryBarHtml(node){
            if(node.runStatus !== 'failed') return '';
        ...
        function requestMetaFromResult(result){
            ...
            const banana = result.banana_request_debug && typeof result.banana_request_debug === 'object' ? result.banana_request_debug : {};
            return {
                provider_id: debug.provider_id || result.provider_id || banana.provider_id || '',
                provider_name: debug.provider_name || result.provider_name || '',
                model: debug.model || result.model || banana.model || '',
                submit_url: debug.submit_url || result.submit_url || banana.submit_url || '',
                task_id: debug.task_id || result.task_id || result.taskId || result.id || '',
                poll_status: debug.poll_status || result.poll_status || result.status || '',
                video_url_count: debug.video_url_count ?? (Array.isArray(result.videos) ? result.videos.length : undefined),
                audio_url_count: debug.audio_url_count ?? (Array.isArray(result.audios) ? result.audios.length : undefined),
                request_id: result.request_id || result.requestId || '',
                backend: result.backend || debug.backend || '',
                workflow: result.workflow_json || result.workflow || '',
            };
        }
        function runPlatformLabel(run){
            const node = run?.node || {};
            const req = run?.request || {};
            const providerId = req.provider_id || req.providerId || node.apiProvider || node.llmProvider || '';
            const provider = providerId ? providerById(providerId) : null;
            if(provider?.name) return provider.name;
            if(providerId) return providerId;
            if(run?.nodeType === 'llm') return 'LLM';
            if(run?.nodeType === 'video') return 'Video';
            return run?.nodeType || '-';
        }
        function runTaskLabel(run){
            const node = run?.node || {};
            const req = run?.request || {};
            return req.model || node.model || node.llmMsModel || node.msCustomModel || '';
        }
        ...
        function renderCanvasLog(){
            ...
            logList.innerHTML = logs.length ? logs.map(log => {
                const thumbs = (log.outputs || []).slice(0, 8).map(item => {
                    ...
                    const kindAttr = ` data-kind="${escapeAttr(kind)}"`;
                    if(isMissingAssetUrl(url)) return `<div class="missing-asset compact" data-url="${safe}"${kindAttr}><i data-lucide="image-off" class="w-4 h-4"></i></div>`;
                    if(kind === 'video') return `<video src="${safe}" data-url="${safe}"${kindAttr} muted playsinline></video>`;
                    if(kind === 'audio') return `<div class="log-audio-thumb" data-url="${safe}"${kindAttr}><i data-lucide="audio-lines" class="w-4 h-4"></i></div>`;
                    return `<img src="${safe}" data-url="${safe}"${kindAttr} alt="output">`;
                }).join('');
            ...
            logList.querySelectorAll('[data-url]').forEach(el => {
                el.onclick = e => {
                    ...
                    openOutputLightbox(el.dataset.url, kind ? {images:[{url:el.dataset.url, kind}]} : null);
                };
            });
        ...
        function mergeGeneratedOutputs(node, outputs, append=false){
            if(!node) return;
            const clean = (outputs || []).filter(item => outputUrlValue(item));
            const keep = node.type === 'video' ? clean : clean.filter(item => mediaKind(item) === 'image');
            if(!append){
                node.generatedOutputs = keep;
                if(node.type === 'video') node.generatedVideos = keep.filter(item => mediaKind(item) === 'video').map(outputUrlValue);
                return;
            }
            const seen = new Set((node.generatedOutputs || []).map(outputUrlValue));
            node.generatedOutputs = [...(node.generatedOutputs || []), ...keep.filter(item => {
                const url = outputUrlValue(item);
                return url && !seen.has(url) && seen.add(url);
            })];
            if(node.type === 'video'){
                const videoSeen = new Set(node.generatedVideos || []);
                const videos = keep.filter(item => mediaKind(item) === 'video').map(outputUrlValue);
                node.generatedVideos = [...(node.generatedVideos || []), ...videos.filter(url => !videoSeen.has(url) && videoSeen.add(url))];
            }
        }
        ...
        function appendOutputImages(out, images, compareRef, metas=[], layout=null){
            const list = (images || []).filter(item => outputUrlValue(item));
            ...
            out.images = [...(out.images || []), ...list.map((entry, i) => {
                const url = outputUrlValue(entry);
                const source = entry && typeof entry === 'object' ? entry : {url};
                const meta = metas[i] || metas[0] || {};
                const item = {...source, url, viewed:false, runMs:meta.runMs || 0, run:meta.run || null};
                if(meta.grid) item.grid = meta.grid;
                return item;
            })];
            if(compareRef?.url){
                out.imageComparisons = out.imageComparisons || {};
                list.map(outputUrlValue).filter(url => mediaKind(url) === 'image').forEach(url => {
        ...
        function outputLightboxItems(out=null){
            ...
            if(sourceOut){
                return outputDisplayItems(sourceOut).map(item => normalize(item, sourceOut)).filter(Boolean);
            }
            const outputNodeItems = nodes
                .filter(n => n.type === 'output')
                .flatMap(n => outputDisplayItems(n).map(item => normalize(item, n)).filter(Boolean));
        ...
        function rerunFromOutputMeta(meta){
            ...
            (meta.run.audios || []).slice(0, 4).forEach((ref, i) => {
                const audioNode = {id:uid('aud'), type:'audio', x:p.x - 340, y:p.y + 110 + (meta.run.refs || []).length * 86 + i * 92, url:ref.url, name:ref.name || 'audio', mime:ref.mime || ''};
                nodes.push(audioNode);
                connections.push({id:uid('c'), from:audioNode.id, to:node.id});
            });
            closeOutputLightbox();
            render();
            scheduleSave();
        ...
        function initOutputPreviewZoomEvents(){
            outputPreview.addEventListener('wheel', e => {
                if(outputLightboxVideo.style.display === 'block' || outputLightboxAudio.style.display === 'block') return;
                e.preventDefault();
                e.stopPropagation();
        ...
        function resetOutputLightboxMedia(){
            if(outputLightboxAudioSyncCleanup){
                outputLightboxAudioSyncCleanup();
                outputLightboxAudioSyncCleanup = null;
            }
            outputLightboxVideo.pause();
            outputLightboxVideo.removeAttribute('src');
            outputLightboxVideo.load?.();
            outputLightboxVideo.style.display = 'none';
            outputLightboxVideo.onloadedmetadata = null;
            outputLightboxAudio.pause();
            outputLightboxAudio.removeAttribute('src');
            outputLightboxAudio.load?.();
            outputLightboxAudio.style.display = 'none';
            outputLightboxImg.src = '';
            outputCompareResult.src = '';
            outputCompareOriginal.src = '';
        }
        function bindOutputLightboxAudioSync(video, audio){
            const syncTime = () => {
                if(Math.abs((audio.currentTime || 0) - (video.currentTime || 0)) > .25){
            ...
            const onRate = () => { audio.playbackRate = video.playbackRate || 1; };
            const onVolume = () => { audio.volume = video.volume; audio.muted = video.muted; };
        ...
        function openOutputLightboxV2(url, out){
            if(!url) return;
            resetOutputPreviewZoom();
            resetOutputLightboxMedia();
            currentOutputLightboxOutId = out?.id || '';
            currentOutputLightboxUrl = url;
            const meta = outputMetaFor(url, out);
            markOutputViewed(out, url);
            ...
            currentOutputCompareUrl = outputCompareUrlFor(url, out);
            setOutputCompareMode(false);
            const kind = mediaKind(meta?.url ? meta : url);
            const isVideo = kind === 'video';
            const isAudio = kind === 'audio';
            outputLightboxImg.style.display = (!isVideo && !isAudio) ? 'block' : 'none';
            outputLightboxVideo.style.display = isVideo ? 'block' : 'none';
            outputLightboxAudio.style.display = isAudio ? 'block' : 'none';
            outputCompareResult.style.display = (!isVideo && !isAudio) ? 'block' : 'none';
            ...
            if(isVideo){
                ...
                outputLightboxVideo.src = url;
                const pairedAudioUrl = meta.audioUrl || meta.audio_url || meta.audio?.url || '';
                if(pairedAudioUrl && pairedAudioUrl !== url){
                    outputLightboxAudio.src = pairedAudioUrl;
                    outputLightboxAudio.style.display = 'none';
                    bindOutputLightboxAudioSync(outputLightboxVideo, outputLightboxAudio);
                }
                outputPreview.ondblclick = null;
                outputLightbox.classList.add('open');
                refreshIcons();
            ...
            if(isAudio){
                outputLightboxAudio.src = url;
                outputResolutionText('Audio', meta);
                outputPreview.ondblclick = null;
                outputLightbox.classList.add('open');
                refreshIcons();
                return;
        ...
        openOutputLightbox = openOutputLightboxV2;
        function closeOutputLightboxV2(){
            outputLightbox.classList.remove('open');
            setOutputCompareMode(false);
            resetOutputLightboxMedia();
            outputLightboxImg.style.display = 'block';
        ...
        closeOutputLightbox = closeOutputLightboxV2;
        ...
        function pasteNodes(){
            ...
            const pasteItems = clipboard.filter(n => !HIDDEN_CANVAS_NODE_TYPES.has(n?.type));
            if(!pasteItems.length) return;
            pushUndo();
            const xs = pasteItems.map(n => n.x), ys = pasteItems.map(n => n.y);
            const cx = (Math.min(...xs) + Math.max(...xs)) / 2;
            const cy = (Math.min(...ys) + Math.max(...ys)) / 2;
            const dx = lastMouseBoard.x - cx;
            const dy = lastMouseBoard.y - cy;
            const idMap = new Map();
            const copies = pasteItems.map(n => { const c = cloneNode(n, dx, dy); idMap.set(n.id, c.id); return c; });
            copies.forEach(c => {
        ...
        function removeHiddenNodesFromCanvas(){
            ...
            nodes = (nodes || []).filter(n => !HIDDEN_CANVAS_NODE_TYPES.has(n?.type));
            connections = (connections || []).filter(c => !ids.has(c.from) && !ids.has(c.to));
            ids.forEach(id => selected.delete(id));
            if(clipboard?.length) clipboard = clipboard.filter(n => !HIDDEN_CANVAS_NODE_TYPES.has(n?.type));
            return true;
        }
        ...
        window.addEventListener('paste', e => {
            ...
            if(audioFiles.length){
                ...
                if(blankAudio) fillAudioNode(blankAudio.id, audioFiles).catch(err => showErrorModal(err.message || tr('canvas.audioUploadFailed')));
                else uploadAudioFiles(audioFiles).catch(err => showErrorModal(err.message || tr('canvas.audioUploadFailed')));
                return;
            }
        ...
        function hasImageFiles(items){
            return [...(items || [])].some(item => {
                if(item?.kind === 'file') return item.type?.startsWith('image/');
                return item?.type?.startsWith('image/');
            });
        }
        function hasAudioFiles(items){
            return [...(items || [])].some(item => {
                if(item?.kind === 'file') return item.type?.startsWith('audio/') || isAudioFile(item.getAsFile?.());
        ...
        function hasAudioDropData(dataTransfer){
            if(!dataTransfer) return false;
