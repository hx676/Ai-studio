...
class ReleaseContractTests(unittest.TestCase):
    ...
    def test_macos_packaging_pipeline_is_present(self):
        ...
        arm64_launcher = (ROOT / "macos" / "SynCanvas-arm64").read_text(encoding="utf-8")
        arm64_manifest = json.loads((ROOT / "macos" / "runtime-arm64.json").read_text(encoding="utf-8"))
        ...
        self.assertIn("arm64-offline", script)
        self.assertIn("macos-arm64", script)
        self.assertIn("dmg-offline", script)
        self.assertIn("--platform macosx_11_0_arm64", script)
        self.assertIn('Write-Utf8NoBom (Join-Path $bootstrapRoot "wheelhouse.sha256")', script)
        ...
        self.assertIn("PIP_NO_INDEX=1", arm64_launcher)
        self.assertIn("wheelhouse.sha256", arm64_launcher)
        self.assertIn("shasum -a 256 -c", arm64_launcher)
        self.assertIn("require_apple_silicon", arm64_launcher)
        self.assertEqual("arm64", arm64_manifest["architecture"])
        self.assertEqual(64, len(arm64_manifest["sha256"]))
